...
@interface YKHttpEngine(PrivateMethods)
...
- (id)init{
    ...
    if (self) {
        ...
        _m_timeInterval_timeout = 3;              // 默认超时20秒
...
-(void)fillupHeaderParams:(ASIFormDataRequest *)_request{
    if (self.m_dict_header!=nil) {
        NSLog(@"HeaderParams: %@", self.m_dict_header==nil?@"":[self.m_dict_header description]);
...
#pragma mark -
#pragma mark 对外接口
+(YKHttpEngine*)engineWithHeaderParams:(NSDictionary *)headerParams{
    YKHttpEngine* _httpEngine = [[YKHttpEngine alloc] init];
...
-(ASIFormDataRequest*)buildRequest:(NSString *)url 
    ...
    NSDictionary* _dict = @{@"object": object,
                           @"finishedAction": finishedActionValue,
                           @"failedAction": failedActionValue};
